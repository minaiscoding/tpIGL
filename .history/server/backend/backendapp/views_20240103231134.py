...
class SaveFavoriteView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not FavoriteArticle.objects.filter(user=user, article_id=article_id).exists():
            favorite_article = FavoriteArticle.objects.create(user=user, article_id=article_id)
            serializer = FavoriteArticleSerializer(favorite_article)
