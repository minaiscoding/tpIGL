...
from .serializers import UtilisateursSerializer, ArticlesSerializer, FavoriteArticleSerializer,UploadArticlesSerializer
#--------------------------------------------------------------------------------
...
class SaveFavoriteView(APIView):
    ...
    def get_article_data(self, article_id):
        try:
            ...
            # Extract relevant information from the search hit
            article_data = {'id': response.hits[0].meta.id, 'title': response.hits[0].Titre}

            print('Article Data:', article_data['id'])  # Add this line for debugging
    ...
    def post(self, request):
        try:
            ...
            print('User name:', utilisateur)
            if not existing_favorite:
                article_data = self.get_article_data(article_id)
                if not article_data:  # If the article doesn't exist
                    return Response({"detail": "Article not found"}, status=status.HTTP_404_NOT_FOUND)
                
                favoris = Favoris(UtilisateurID=utilisateur, ArticleID=article_data['title'])
                favoris.save()

                serializer = FavoriteArticleSerializer(favoris)
...
class SaveFavoriteViewTest(TestCase):
    def setUp(self):
        # Create a user for testing
        self.user = Utilisateurs.objects.create(NomUtilisateur='testuser', Email='test@example.com', MotDePasse='testpassword', Role='user')

        # Create an article for testing
        self.article = Articles.objects.create(id='1', Titre='Test Article', Resume='Test Resume', auteurs='Test Author', date='2022-01-01', MotsCles='Test Keyword', text='Test Text', URL_Pdf='https://example.com/test.pdf', RefBib='Test Bib')

    def test_existing_favorite_filter(self):
        # Create a Favoris instance for the user and the existing article
        favoris = Favoris.objects.create(UtilisateurID=self.user, ArticleID=self.article)

        # Mock a request with the necessary data
        request_data = {
            "articleId": '1',
            "userId": self.user.id,
        }

        # Create an instance of the SaveFavoriteView
        view = SaveFavoriteView()

        # Set the request data for the view
        view.request = view.initialize_request(view, method='POST', data=request_data)

        # Call the post method to test the filter
        response = view.post(view.request)

        # Check if the filter works correctly
        self.assertEqual(response.status_code, 400)  # Assuming you return a 400 response if the article is already favorited
        self.assertEqual(response.data['detail'], 'Article is already marked as a favorite.')
#-----------------------------------------------------------------------------------------------------
...
class FavoriteArticleListView(APIView):
     ...
     def get(self, request, user_id):
        ...
        search = Search(index='articles').query('terms', id=article_ids)
        response = search.execute()
