...
from .serializers import UtilisateursSerializer, ArticlesSerializer,UploadArticlesSerializer, FavorisSerializer
#--------------------------------------------------------------------------------
...
@csrf_exempt
def AddTo_favorite(request, user_id, article_id):
    if request.method == 'POST':
        try:
            # Assuming article_id is a CharField in your Articles model
            article = Articles.objects.get(id=article_id)

            # Use the serializer to validate and save the data
            serializer = FavorisSerializer(data={'UtilisateurID': user_id, 'ArticleID': article.id})
            if serializer.is_valid():
                serializer.save()
