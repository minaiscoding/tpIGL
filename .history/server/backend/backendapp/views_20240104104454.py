...
class SaveFavoriteView(APIView):
    ...
    def get_article_data(self, article_id):
        try:
            ...
            # Extract only the title from the search hit
            title = response.hits[0].Titre  # Replace 'titre' with the actual field name
            print('Article Title:', title)  # Add this line for debugging
    ...
    def post(self, request):
        try:
            ...
            article_title = self.get_article_data(article_id)
                
                if not article_title:  # If the article doesn't exist
                    return Response({"detail": "Article not found"}, status=status.HTTP_404_NOT_FOUND)

            existing_favorite = Favoris.objects.filter(
                UtilisateurID=utilisateur, ArticleID=article_data['id']
            ).exists()
            
            if not existing_favorite:
                
                favoris = Favoris(UtilisateurID=utilisateur, ArticleID=article_data['id'])
                favoris.save()
