...
class SaveFavoriteView(APIView):
    ...
    def get_article_data(self, article_id):
        try:
            # Check if the article with the given ID exists in Articles
            existing_article = Articles.objects.filter(id=article_id).first()

            if existing_article:
                return existing_article.to_dict()  # Adjust this based on your serialization logic
            else:
            # Perform the Elasticsearch search to get the specific article by ID
                search = Search(index='articles').query('match_all')
                response = search.execute()
                print('Elasticsearch Response for all articles:', response.to_dict())

        # Extract relevant information from search hits
                hits = [{'id': hit.meta.id, **hit.to_dict()} for hit in response.hits]

        # Filter the articles based on the provided article_id
                filtered_articles = [article for article in hits if article['id'] == article_id]
        ...
        # Assuming there's only one matching article (ID is unique)
                article_data = filtered_articles[0]
                return article_data
    ...
    def post(self, request):
        try:
            ...
            if not existing_favorite:
                ...
                # Use the existing article instance or create a new one
                article_instance, created = Articles.objects.get_or_create(id=article_id, defaults=article_data)
                
                favoris = Favoris(UtilisateurID=utilisateur, ArticleID=# Use the existing article instance or create a new one
                article_instance, created = Articles.objects.get_or_create(id=article_id, defaults=article_data))
                favoris.save()
