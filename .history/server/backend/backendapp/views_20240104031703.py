...
from .views import OneArticleView
#--------------------------------------------------------------------------------------------------------------
...
class OneArticleView(APIView):
    ...
    def get(self, request, article_id):
        ...
        search = Search(index='articles').query('match', _id=article_id)
        response = search.execute()
        ...
        # Extract relevant information from the search hit
        article_data = {'id': response.hits[0].meta.id, **response.hits[0].to_dict()}

        # Serialize the article data using your existing serializer
        serializer = ArticlesSerializer(data=article_data)
        serializer.is_valid()

        # Return the serialized article data as JSON
        return Response(serializer.data)
    
class SaveFavoriteView(APIView):

    def get_article_data(self, article_id):
        # Call the helper method to retrieve article data by ID
        response = ArticlesListView().get(self.request, pk=article_id)  # Pass the article ID to the get method
    ...
    def post(self, request):
        try:
            ...
            if not existing_favorite:
                # If not, retrieve the article data using the helper method
                article_data = self.get_article_data(article_id)

                # Check if the article with the given ID exists
                if "detail" in article_data:
                    return Response(article_data, status=status.HTTP_404_NOT_FOUND)
