  ...
  const handleSearch = async () => {
    if (query.trim() !== '') {
      try {
        ...
        // Navigate to SearchResult page with search results and query
        navigate('/search-result', { state: { results: data, query: query } });
      } catch (error) {
...
<div className="absolute translate-y-[18vh] h-[200px] flex items-center justify-center z-20 w-[50vw]">
  ...
  />
  <button onClick={handleSearch} className="bg-gray-800 text-white px-[6%] py-3 rounded-r-md">
    Search
  </button>
</div>
