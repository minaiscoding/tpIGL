...
from .serializers import UtilisateursSerializer, ArticlesSerializer,UploadArticlesSerializer
#--------------------------------------------------------------------------------
...
from django.http import JsonResponse
from random import choice  # Import the choice function to pick a random item from a list


def get_random_user_id(request):
    user_ids = Utilisateurs.objects.values_list('id', flat=True)  # Get all user IDs from the database
    random_user_id = choice(user_ids)  # Pick a random user ID
    
    return JsonResponse({'user_id': random_user_id})
...
def AddToFavorite(request, user_id, article_id):
    
    user = get_object_or_404(Utilisateurs, id=user_id)
    article = get_object_or_404(Articles, id=article_id)  

    favorite, created = Favoris.objects.get_or_create(user=user, article=article)

    if created:
        return Response({'message': 'Article added to favorites successfully'})
    else:
        return Response({'message': 'Article is already in favorites'})    
