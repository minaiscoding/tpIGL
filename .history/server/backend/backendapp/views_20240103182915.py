...
class FavoriteArticleListView(ListAPIView):
    ...
    def get_user_favorite_article_ids(request, user_id):
    # Retrieve the user's favorite articles IDs
    favorite_articles = Favoris.objects.filter(user_id=user_id)
    article_ids = [fav.article.id for fav in favorite_articles]

    # Return the article IDs in the response
