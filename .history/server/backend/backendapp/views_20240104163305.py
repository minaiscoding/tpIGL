...
class SaveFavoriteView(APIView):
    ...
    def post(self, request):
        try:
            ...
            print('l''articleeeeeee', article_data)
            if not article_data:
                    return Response({"detail": "Article not found"}, status=status.HTTP_404_NOT_FOUND)

            article_instance, created = Articles.objects.get_or_create(
                    id=article_data['id'],
                    defaults=article_data  # Use article_data as defaults when creating
            )

            existing_favorite = Favoris.objects.filter(
                UtilisateurID=utilisateur,
                ArticleID=article_instance  # Filter by the article ID
            ).exists()
...
class FavoriteArticleListView(APIView):
     ...
     def get(self, request, user_id):
        ...
        # Serialize the search results using your existing serializer
        serializer = FavoriteArticleSerializer(data=hits, many=True)
        serializer.is_valid()
